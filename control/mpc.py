...
CFG = config.MPC_CONFIG

# Model parameters — from persistent store, floor area always from config
MODEL_PARAMS        = _model_store.load()
MODEL_PARAMS["A"]   = config.MPC_MODEL["A"]
...
# Default initial state [Ti, Tm] when no state file exists
_DEFAULT_X0 = config.MPC_MODEL["x0"].flatten()
...
def reload_model() -> None:
    ...
    MODEL_PARAMS["A"] = config.MPC_MODEL["A"]
    _discrete_model_cache = None
    logger.info("🔄 MPC model reloaded from model_store")
...
def get_forecasts(horizon_steps: int) -> dict | None:
    ...
    # ── Schedule ─────────────────────────────────────────────────────────────
    sched_h = schedules.get_setpoint_horizon(steps=horizon_steps)
    if not sched_h:
...
def run_mpc_step(save_plot_flag: bool = True, max_heat: float = 1000.0) -> float | None:
    """
    Run one complete MPC step.

    Flow:
        1. Read indoor temperature and previous heat output
        2. Kalman filter update  → posterior state x̂[k|k]
        3. Fetch forecasts
        4. Solve LP              → optimal heating sequence
        5. Save forecast plot    (optional)

    Returns:
        Optimal setpoint [°C], or None on failure
    """
    logger.info("🚀 Running MPC step...")

    # --- Measurements ---
    room     = state.get_by_role("room_temp")
    T_indoor = room.get("temperature") if room else None
    if T_indoor is None:
        logger.error("❌ No indoor temperature")
        return None

    derived = state.get_derived("radiator_1_output")
    ...
    # --- Forecasts ---
    forecasts = get_forecasts(HORIZON_STEPS)
...
def get_mpc_status() -> dict:
    """Get MPC status summary for Telegram bot."""
    s = get_state()
    return {
...
def get_mpc_plot_path() -> str | None:
    """Get path to latest forecast chart HTML, or None."""
    s    = get_state()
